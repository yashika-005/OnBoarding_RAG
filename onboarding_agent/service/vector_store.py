...
from onboarding_agent.core.config import settings


class VectorStoreManager:
    """Manages vector stores for document similarity search"""
    ...
    def load_existing_stores(self) -> dict:
        ...
        try:
            if self._db_exists(settings.GRATUITY_DB_PATH):
                self.gratuity_db = Chroma(
                    persist_directory=settings.GRATUITY_DB_PATH,
        ...
        try:
            if self._db_exists(settings.LEAVE_DB_PATH):
                self.leave_db = Chroma(
                    persist_directory=settings.LEAVE_DB_PATH,
                    embedding_function=self.embeddings
                )
        ...
        try:
            if self._db_exists(settings.UPSKILLING_DB_PATH):
                self.upskilling_db = Chroma(
                    persist_directory=settings.UPSKILLING_DB_PATH,
                    embedding_function=self.embeddings
                )
    ...
    def get_relevant_documents(self, query: str) -> Tuple[Optional[List[Document]], str]:
        ...
        return [doc for doc, score in best_results if score >= settings.SIMILARITY_THRESHOLD], best_source
